...
export const DesktopSidebar = ({
  ...
  return (
    <>
      <motion.div
        className={cn(
          "h-full px-3.5 py-5 hidden  md:flex md:flex-col bg-zinc-100 dark:bg-zinc-900 w-[300px] shrink-0",
          className
        )}
        animate={{
...
export const MobileSidebar = ({
  ...
  return (
    <>
      <div
        className={cn(
          "h-28 px-4 py-4 flex flex-row md:hidden  items-center justify-between bg-zinc-100 dark:bg-zinc-900 w-full"
        )}
        {...props}
      >
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              <div
                className="absolute right-10 top-10 z-50 text-zinc-800 dark:text-zinc-200"
                onClick={() => setOpen(!open)}
              >
                <IconX />
              </div>
